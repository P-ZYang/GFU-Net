...
parser.add_argument("--model_names", default=['Ours'], type=list, help="'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'RISTDnet'")
...
parser.add_argument("--log_dir", type=str, default="./otherlogs/Ours", help='path of log files')
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        if model_name == 'Ours':
            if mode == 'train':
                self.model = Ours(config_vit, mode='train', deepsuper=True)
            else:
                self.model = Ours(config_vit, mode='test', deepsuper=True)
